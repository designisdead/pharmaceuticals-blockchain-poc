...
async function transferDrug(drugTransfer) {
    ...
    transferEvent.fromId = drugProduction.manufacturer.getIdentifier();
    transferEvent.toId = getCurrentParticipant().getIdentifier();
    emit(transferEvent);
...
async function receiveDrug(reception) {
    ...
    if (asset.owner.getIdentifier() !== invoker) {
        ...
        receptionEvent.serialNumber = drugProduction.serialNumber;
