 ...
 * @param {org.howest.mydrugchain.Verification} verification
 * @transaction
 */
async function receiveDrug(verification) {
    let factory = getFactory();
    let NS = 'org.howest.mydrugchain';

    //verify metadata by checking hash attribute
    let checkHash = hash(verification.serialNumber+verification.productCode+verification.batchNumber+verification.manufacturerId);
    let assetRegistry = await getAssetRegistry('org.howest.mydrugchain.Drug');
    let asset = await assetRegistry.get(verification.serialNumber);

    //create a new transaction depending on the provided drug hash
    let trxId = guid();
    //work with concept instead

    if (asset.productHash === checkHash) {
        ...
        let quarantine = factory.newResource(NS, 'QuarantinePlacement',trxId);
        quarantine.start = verification.receptionDate;
        quarantine.reason = 'provided data do not match with hash!';
        quarantine.drug = factory.newRelationship(NS, 'Drug', asset.serialNumber);
        let trxRegistry = await getTransactionRegistry('org.howest.mydrugchain.QuarantinePlacement');
        await trxRegistry.add(quarantine);
    }
